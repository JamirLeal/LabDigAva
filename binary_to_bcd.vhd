...
BEGIN
	...
	BEGIN
		IF(reset_n = '0') THEN						--asynchronous reset asserted
			bit_count := 0;								--reset bit counter								--indicate not available
		...
		ELSIF(clk'EVENT AND clk = '1') THEN		--system clock rising edge
			CASE state IS
				...
				WHEN idle =>							--idle state
					IF(ena = '1') THEN					--converter is enabled						--indicate conversion in progress
					...
					ELSE										--converter is not enabled							--indicate available
						converter_ena <= '0';				--disable the converter
				...
				WHEN convert =>													--convert state
					...
					ELSE																	--all bits shifted in													--indicate conversion is complete
						converter_ena <= '0';											--disable the converter
